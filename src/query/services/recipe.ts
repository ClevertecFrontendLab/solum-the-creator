...
export const recipeApiSlice = apiSlice
    ...
    .injectEndpoints({
        endpoints: (builder) => ({
            [EndpointNames.GET_RECIPES]: builder.query<RawRecipe[], number>({
                query: (limit) => ({
                ...
                transformResponse: (response: { data: RawRecipe[] }): RawRecipe[] =>
...
export const { useGetRecipesQuery, useLazyGetRecipesQuery } = recipeApiSlice;
